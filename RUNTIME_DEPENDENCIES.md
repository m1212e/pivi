...
| **Invidious** (+ invidious-companion, its own postgres) | Browse/search backend for the YouTube plugin (`plugins/youtube/invidious.ts`) — direct authenticated InnerTube browse/search calls don't work reliably (see SKETCH.md's YouTube plugin notes), so this self-hosted instance does that instead. Runs as three extra `docker-compose.yaml` services (`invidious`, `invidious-companion`, `invidious-db`), sourced from the `vendor/invidious` git submodule for its DB init scripts. | `docker compose up -d invidious invidious-companion invidious-db`                               | same                                                                                                                                                                                          |

Note: `vendor/invidious` is a git submodule (only its `config/sql` and
`docker/init-invidious-db.sh` are actually used, to keep the Invidious DB
schema in sync with whatever image tag is running) — a fresh clone needs
`git submodule update --init` before `docker compose up` will find those
files. `INVIDIOUS_HMAC_KEY`/`INVIDIOUS_COMPANION_KEY` (see `.env.example`)
are internal secrets for the invidious/invidious-companion pair, not
credentials for anything external — generate with `openssl rand -hex 8`
(Invidious requires exactly 16 characters, so `-hex 8`, not `-hex 16`).
